...
export const Hero: React.FC<HeroProps> = ({
  ...
  return (
    <section
      id="hero"
      className="relative min-h-screen w-full flex flex-col justify-between items-center pt-28 pb-10 px-4 sm:px-6 lg:px-8 overflow-hidden bg-[#faf9f6]"
    >
      ...
      <div className="relative z-20 w-full max-w-3xl text-center my-auto flex flex-col items-center justify-center space-y-3.5 sm:space-y-8 px-2 sm:px-0">
        ...
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/90 border border-stone-200 shadow-xs backdrop-blur-xs text-xs font-medium text-stone-700 hover:bg-white transition-all">
          ...
          <span>Compléments Alimentaires en Gummies</span>
        </div>
        ...
        <p className="font-sans text-xs sm:text-xl text-stone-600 max-w-2xl leading-relaxed font-normal px-2 sm:px-0">
          5 formules hautement concentrées, enrichies en vitamines essentielles et extraits botaniques. Conçues avec précision suisse pour réinventer votre santé au quotidien.
        </p>
        ...
        <div className="w-full max-w-full pt-2">
          <div className="w-full overflow-x-auto no-scrollbar py-2.5 px-4 pr-8 sm:px-6 flex items-center justify-start sm:justify-center gap-2.5 flex-nowrap touch-pan-x">
            {MOLITA_PRODUCTS.map((p, idx) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 text-xs pl-1.5 pr-3.5 py-1.5 rounded-full bg-white/90 border text-stone-800 hover:border-stone-400 active:scale-95 transition-all flex items-center gap-2 shadow-2xs whitespace-nowrap cursor-pointer ${
                    isActiveMobile
                      ? 'border-stone-800 ring-2 ring-stone-900/10 font-bold'
                      : 'border-stone-200'
                  }`}
                >
                  <div
