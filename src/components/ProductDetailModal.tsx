      ...
      >
        ...
        <div className="p-6 sm:p-8 space-y-6 overflow-y-auto font-sans">
          ...
          <div className="p-5 rounded-3xl bg-white border border-stone-200/80 shadow-xs flex flex-col sm:flex-row items-center gap-6">
            ...
            <div className="space-y-2 text-center sm:text-left flex-1">
              ...
              <div className="pt-2 flex flex-wrap items-center justify-center sm:justify-start gap-x-4 gap-y-1 text-xs text-stone-600">
                <span><strong>Goût:</strong> {product.taste}</span>
                <span>•</span>
                <span><strong>Contenance:</strong> {product.countPerBottle} gummies (Cure 1 mois)</span>
                <span>•</span>
                <span><strong>Prix:</strong> CHF {product.priceCHF}.00</span>
              </div>
            </div>
          </div>

          {/* Grid Layout: Key Benefits & Vitamins */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-5 rounded-2xl border border-stone-200/70 shadow-xs space-y-3">
              <h4 className="font-serif text-base font-semibold text-stone-900 flex items-center gap-2">
                ...
                Vitamines & Actifs Clés
              </h4>
              <div className="flex flex-wrap gap-1.5 pt-1">
                {product.vitamins.map((vit, idx) => (
                  <span
                    key={idx}
                    className="text-xs font-medium px-2.5 py-1 rounded-lg bg-stone-100 text-stone-800 border border-stone-200"
                  >
                    {vit}
                  </span>
                ))}
                {product.keyIngredients.map((ing, idx) => (
                  <span
                    ...
                    style={{
                      backgroundColor: `${product.accentHex}15`,
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-stone-100/70 border border-stone-200 text-xs">
              <span className="font-semibold text-stone-900 block mb-1">🎯 Pour qui ?</span>
              <p className="text-stone-600 leading-relaxed">{product.targetAudience}</p>
            </div>
            <div className="p-4 rounded-xl bg-stone-100/70 border border-stone-200 text-xs">
              <span className="font-semibold text-stone-900 block mb-1 flex items-center gap-1">
                <Clock className="w-3.5 h-3.5 text-stone-500" /> Posologie & Conseils
              </span>
              <p className="text-stone-600 leading-relaxed">{product.dosage}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 text-xs text-stone-500 pt-2 border-t border-stone-200">
            ...
            <span>
              Produit certifié conforme aux normes suisses. Distribution exclusive par conseiller téléphonique dédié.
            </span>
          </div>
        </div>

        {/* Modal Footer / Order & Call CTA */}
        <div className="p-6 bg-stone-100/90 border-t border-stone-200 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <span className="text-xs text-stone-500 block">Prix indicatif Suisse</span>
            <span className="text-2xl font-serif font-bold text-stone-900">
              CHF {product.priceCHF}.00 <span className="text-xs font-sans font-normal text-stone-500">/ flacon (60 gummies)</span>
            </span>
          </div>
          ...
          >
            ...
            Être rappelé pour {product.name}
          </button>
        </div>
      </div>
    </div>
